...
"""
# Contenu complet du fichier app/barcode_detector.py
...
# ... (fonctions is_gs1_data, has_datamatrix_characteristics, etc. inchangées) ...
...
def is_gs1_data(raw_data):
    ...
    if re.match(ai_pattern, raw_data) and re.fullmatch(r"[\dA-Z\-\.\/\x1d]+", raw_data, re.IGNORECASE):
         if not raw_data.lower().startswith(("http:", "https:", "www.")): return True
    return False
...
def detect_generic_format(raw_data):
    ...
    if has_datamatrix_characteristics(raw_data): return BarcodeFormat.DATAMATRIX
    if has_qrcode_characteristics(raw_data): return BarcodeFormat.QRCODE
    if re.fullmatch(r"[\w\s!\"#$%&'()*+,-./:;<=>?@\[\\\]^_`{|}~]+", raw_data):
         if not has_datamatrix_characteristics(raw_data) and not has_qrcode_characteristics(raw_data): return BarcodeFormat.CODE128
    return BarcodeFormat.UNKNOWN

def calculate_confidence(data, decoder, format_enum):
    base_score = 0.0
    if decoder == DecoderType.ZXING: base_score = 0.9
    elif decoder == DecoderType.PYLIBDMTX: base_score = 0.7
    elif decoder == DecoderType.TEXT_INPUT: base_score = 0.95
    else: base_score = 0.5
    if format_enum in [BarcodeFormat.GS1_128, BarcodeFormat.GS1_DATAMATRIX, BarcodeFormat.GS1_QRCODE]:
        if '\x1d' in data: base_score = min(base_score + 0.2, 1.0)
        elif re.match(r"^(01|02|10|11|17|21|30|37|4\d{2})", data): base_score = min(base_score + 0.05, 1.0)
    if format_enum != BarcodeFormat.UNKNOWN: base_score = min(base_score + 0.1, 1.0)
...
def get_barcode_characteristics(data, format_enum):
    ...
    Version corrigée pour une meilleure détection des AI potentiels.
    """
    ...
    characteristics = {
        ...
        "contains_fnc1": contains_fnc1
    }

    # Caractéristiques spécifiques aux formats GS1
    if format_enum in [BarcodeFormat.GS1_128, BarcodeFormat.GS1_DATAMATRIX, BarcodeFormat.GS1_QRCODE]:
        # Regex amélioré qui cherche les AI connus sans se soucier des "limites de mot"
        # Il cherche les préfixes AI qui sont typiquement au début d'un segment de données.
        potential_ai_pattern = r"(00|01|02|10|11|13|15|17|20|21|240|250|251|253|254|255|30|310[0-9]|311[0-9]|37|400|401|410|414|420|422|700[1-3]|800[1-8]|8020|9[0-9])"
        ai_candidates = re.findall(potential_ai_pattern, data)
